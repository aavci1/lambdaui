...
public:
  ...
  virtual bool updateRgbaPixels(std::span<std::uint8_t const> rgbaPixels, WGPUDevice webGpuDevice = nullptr);
  ...
  /// `webGpuDevice` is an optional WGPUDevice for eager upload.
...
/// `webGpuDevice` must match the target canvas device when eager upload is required.
