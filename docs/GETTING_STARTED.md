...
WebGPU APIs are always available. `Canvas::webGpuDevice()`, `Canvas::webGpuQueue()`, `Canvas::webGpuRenderTargetFormat()`, and `webGpuCanvasHandles(canvas)` expose borrowed Dawn handles for resource creation. WebGPU render targets use `WebGpuRenderTargetSpec`; set its `device` and `textureView` fields to render into a caller-owned Dawn/WebGPU texture view.
