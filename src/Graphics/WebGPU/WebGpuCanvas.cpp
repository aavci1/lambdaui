...
#ifndef WGPU_BIND_GROUP_DESCRIPTOR_INIT
#define WGPU_BIND_GROUP_DESCRIPTOR_INIT {}
#endif
#ifndef WGPU_BIND_GROUP_ENTRY_INIT
#define WGPU_BIND_GROUP_ENTRY_INIT {}
#endif
#ifndef WGPU_BIND_GROUP_LAYOUT_DESCRIPTOR_INIT
#define WGPU_BIND_GROUP_LAYOUT_DESCRIPTOR_INIT {}
#endif
#ifndef WGPU_BIND_GROUP_LAYOUT_ENTRY_INIT
#define WGPU_BIND_GROUP_LAYOUT_ENTRY_INIT {}
#endif
#ifndef WGPU_BLEND_STATE_INIT
#define WGPU_BLEND_STATE_INIT {}
#endif
#ifndef WGPU_BUFFER_DESCRIPTOR_INIT
#define WGPU_BUFFER_DESCRIPTOR_INIT {}
#endif
#ifndef WGPU_COLOR_TARGET_STATE_INIT
...
#endif
#ifndef WGPU_EXTENT_3D_INIT
#define WGPU_EXTENT_3D_INIT {}
#endif
#ifndef WGPU_FRAGMENT_STATE_INIT
#define WGPU_FRAGMENT_STATE_INIT {}
#endif
#ifndef WGPU_PIPELINE_LAYOUT_DESCRIPTOR_INIT
...
#endif
#ifndef WGPU_RENDER_PIPELINE_DESCRIPTOR_INIT
#define WGPU_RENDER_PIPELINE_DESCRIPTOR_INIT {}
#endif
#ifndef WGPU_SAMPLER_DESCRIPTOR_INIT
#define WGPU_SAMPLER_DESCRIPTOR_INIT {}
#endif
#ifndef WGPU_SHADER_MODULE_DESCRIPTOR_INIT
...
#endif
#ifndef WGPU_TEXEL_COPY_BUFFER_INFO_INIT
#define WGPU_TEXEL_COPY_BUFFER_INFO_INIT {}
#endif
#ifndef WGPU_TEXEL_COPY_BUFFER_LAYOUT_INIT
#define WGPU_TEXEL_COPY_BUFFER_LAYOUT_INIT {}
#endif
#ifndef WGPU_TEXEL_COPY_TEXTURE_INFO_INIT
#define WGPU_TEXEL_COPY_TEXTURE_INFO_INIT {}
#endif
#ifndef WGPU_TEXTURE_DESCRIPTOR_INIT
#define WGPU_TEXTURE_DESCRIPTOR_INIT {}
#endif
#ifndef WGPU_TEXTURE_VIEW_DESCRIPTOR_INIT
#define WGPU_TEXTURE_VIEW_DESCRIPTOR_INIT {}
#endif
#ifndef WGPU_VERTEX_ATTRIBUTE_INIT
#define WGPU_VERTEX_ATTRIBUTE_INIT {}
#endif
#ifndef WGPU_VERTEX_BUFFER_LAYOUT_INIT
...
#endif
#define wgpuDeviceAddRef wgpuDeviceReference
#endif
...
inline constexpr WGPUMapAsyncStatus WGPUMapAsyncStatus_Error = WGPUBufferMapAsyncStatus_Error;
inline constexpr WGPUMapAsyncStatus WGPUMapAsyncStatus_Success = WGPUBufferMapAsyncStatus_Success;
#endif
...
#if LAMBDAUI_DAWN_LEGACY_NATIVE
void frameCaptureMapCallback(WGPUMapAsyncStatus status, void* userdata) {
  auto* state = static_cast<WebGpuMapWaitState*>(userdata);
  state->status = status;
  state->done = true;
}
#else
...
void replaceAttribute(std::string& text, std::string_view name) {
  std::string const marker = "@" + std::string(name) + "(";
  std::size_t pos = 0;
  while ((pos = text.find(marker, pos)) != std::string::npos) {
    std::size_t const close = text.find(')', pos + marker.size());
    if (close == std::string::npos) {
      return;
    }
    std::string const replacement = "[[" + text.substr(pos + 1, close - pos) + "]]";
    text.replace(pos, close - pos + 1, replacement);
    pos += replacement.size();
  }
}

std::string legacyWgslSource(char const* source) {
  std::string text = source ? std::string(source) : std::string{};

  bool inStruct = false;
  std::size_t lineStart = 0;
  while (lineStart < text.size()) {
    std::size_t const lineEnd = text.find('\n', lineStart);
    std::size_t const end = lineEnd == std::string::npos ? text.size() : lineEnd;
    std::string_view line(text.data() + lineStart, end - lineStart);
    if (line.find("struct ") != std::string_view::npos && line.find('{') != std::string_view::npos) {
      inStruct = true;
    }
    if (inStruct && end > lineStart && text[end - 1] == ',') {
      if (line.find(": ") != std::string_view::npos) {
        text[end - 1] = ';';
      }
    }
    if (inStruct && line.find("};") != std::string_view::npos) {
      inStruct = false;
    }
  ...
  replaceAll(text, "struct Rects {", "[[block]] struct Rects {");
  replaceAll(text, "struct Quads {", "[[block]] struct Quads {");
  replaceAll(text, "struct FrameUniforms {", "[[block]] struct FrameUniforms {");
  replaceAll(text, "switch index {", "switch (index) {");
  replaceAll(text, "smoothstep(", "lambda_smoothstep(");
  replaceAll(text, "@vertex", "[[stage(vertex)]]");
  replaceAll(text, "@fragment", "[[stage(fragment)]]");
  replaceAttribute(text, "group");
  replaceAttribute(text, "binding");
  replaceAttribute(text, "builtin");
  replaceAttribute(text, "location");
  replaceAttribute(text, "interpolate");
  text.insert(0,
              "fn lambda_smoothstep(edge0: f32, edge1: f32, x: f32) -> f32 {\n"
              "  let t = clamp((x - edge0) / max(edge1 - edge0, 0.000001), 0.0, 1.0);\n"
...
WGPUShaderModule createWgslShader(WGPUDevice device, char const* label, char const* source) {
#if LAMBDAUI_DAWN_LEGACY_NATIVE
  std::string legacySource = legacyWgslSource(source);
  WGPUShaderModuleWGSLDescriptor wgsl{};
  wgsl.chain.sType = WGPUSType_ShaderModuleWGSLDescriptor;
  wgsl.source = legacySource.c_str();
...
public:
  ...
  void setResizeBoundsHint(int logicalWidth, int logicalHeight) noexcept {
    int const nextWidth = std::max(0, logicalWidth);
    int const nextHeight = std::max(0, logicalHeight);
    if (resizeBoundsWidth_ == nextWidth && resizeBoundsHeight_ == nextHeight) {
      return;
    }
    resizeBoundsWidth_ = nextWidth;
    resizeBoundsHeight_ = nextHeight;
    if (surface_) {
      configureSurface();
    }
  }
...
private:
  ...
  void encodeFramePass() {
    ensureFrameObjects();
    std::uint64_t rectOps = 0;
    std::uint64_t imageOps = 0;
    std::uint64_t glyphOps = 0;
    std::uint64_t pathOps = 0;
    ...
    debug::perf::recordFrameOps(rectOps,
                                imageOps,
                                pathOps,
                                glyphOps,
                                drawOps_.size(),
        ...
        switch (op.kind) {
          ...
          case WebGpuDrawOp::Kind::Image:
            ...
            try {
              ...
              wgpuRenderPassEncoderDraw(pass, 6, op.count, 0, op.first);
              debug::perf::recordDrawCall(op.isGlyph ? debug::perf::RenderCounterKind::Glyph
                                                      : debug::perf::RenderCounterKind::Image);
  ...
  void finishFrameCaptureReadback() {
...
#if LAMBDAUI_DAWN_LEGACY_NATIVE
    wgpuBufferMapAsync(pendingFrameCaptureBuffer_,
                       WGPUMapMode_Read,
                       0,
                       static_cast<std::size_t>(pendingFrameCaptureBufferSize_),
                       frameCaptureMapCallback,
                       &mapState);
    for (int attempts = 0; attempts < 10000 && !mapState.done; ++attempts) {
      wgpuDeviceTick(context_.device());
    }
    ...
    pendingFrameCaptureMapped_ = true;
  ...
  void clearPendingFrameCapture() noexcept {
    ...
    pendingFrameCaptureMapped_ = false;
  ...
  WGPURenderPipeline ensureRectPipeline(BlendMode blendMode) {
    ...
    WGPUShaderModule shader = createWgslShader(context_.device(),
                                               "LambdaUI WebGPU Rect Shader",
                                               kRectShaderWgsl);
    if (!shader) {
  ...
  bool pendingFrameCaptureMapped_ = false;
};
