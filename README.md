...
`Canvas::webGpuDevice()`, `Canvas::webGpuQueue()`, and `Canvas::webGpuRenderTargetFormat()` return borrowed Dawn handles. Pass the device and queue together to image upload helpers when eager upload is needed. `WebGpuRenderTargetSpec` can allocate an internal target on a supplied device/queue or render into a caller-owned `WGPUTextureView`.
